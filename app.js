...
const calc=p=>{let q=+p.qty_sold||0,rq=Math.max(0,+p.rebate_qualified||0),u=Math.max(1,+p.units_required||1),r=+p.rebate_amount||0,price=+p.item_price||0;let qual=Math.floor(rq/u);return{sales:q*price,qual,due:qual*r}};
...
$("saveProgram").onclick=async e=>{e.preventDefault();let p={user_id:session.user.id,period_id:pid,plu:$("plu").value.trim(),description:$("desc").value.trim(),size_pack:$("size").value.trim(),scan_program:$("scan").value.trim(),rebate_amount:+$("rebate").value||0,start_date:$("start").value,end_date:$("end").value,units_required:+$("units").value||1,qty_sold:+$("qty").value||0,rebate_qualified:+$("rebateQualified").value||0,item_price:+$("price").value||0,notes:$("notes").value.trim(),updated_at:new Date().toISOString()};if(!p.plu||!p.description||!p.start_date||!p.end_date)return alert("PLU, description and dates are required");if(p.rebate_qualified>p.qty_sold)return alert("Rebate Qualified Qty cannot be greater than Qty Sold.");let id=$("programId").value,res=id?await S.from("rebate_programs").update(p).eq("id",id):await S.from("rebate_programs").insert(p);if(res.error)return alert(res.error.message);$("programDlg").close();await loadPrograms()}
...
function addSummarySheet(wb, period){
  ...
  const totals=programs.reduce((a,p)=>{const c=calc(p);a.qty+=+p.qty_sold||0;a.rq+=+p.rebate_qualified||0;a.sales+=c.sales;a.qual+=c.qual;a.due+=c.due;return a},{qty:0,rq:0,sales:0,qual:0,due:0});

  ws.mergeCells("A8:D8"); ws.getCell("A8").value="SUMMARY";
  ...
  const metrics=[
    ...
    ["TOTAL QUALIFYING REDEMPTION",totals.qual],
    ["TOTAL DUE",totals.due]
  ];
  metrics.forEach((m,i)=>{
  ...
  const headers=["PLU","Scan Program","Description","Size / Pack","Units Required","Redemption Amt","Start Date","End Date","Qty Sold","Rebate Qualified","Sales Total","Qualifying Redemption","Total Due"];
  ...
  programs.slice().sort((a,b)=>String(a.plu).localeCompare(String(b.plu),undefined,{numeric:true})).forEach((p,i)=>{
    ...
    const vals=[p.plu,p.scan_program||"",p.description,p.size_pack||"",+p.units_required,+p.rebate_amount,excelDate(p.start_date),excelDate(p.end_date),+p.qty_sold,+p.rebate_qualified||0,c.sales,c.qual,c.due];
    vals.forEach((v,j)=>ws.getCell(r,j+1).value=v);
    ws.getCell(r,6).numFmt='$#,##0.00'; ws.getCell(r,11).numFmt='$#,##0.00'; ws.getCell(r,13).numFmt='$#,##0.00';
    ws.getCell(r,13).font={bold:true,color:{argb:"FF087A3E"}};
  });
  const tr=17+programs.length;
  ws.getCell(tr,8).value="TOTALS"; ws.getCell(tr,9).value=totals.qty; ws.getCell(tr,10).value=totals.rq; ws.getCell(tr,11).value=totals.sales; ws.getCell(tr,12).value=totals.qual; ws.getCell(tr,13).value=totals.due;
  for(let c=1;c<=12;c++){ws.getCell(tr,c).fill={type:"pattern",pattern:"solid",fgColor:{argb:"FFE0F2EF"}};ws.getCell(tr,c).font={bold:true};}
  ws.getCell(tr,11).numFmt='$#,##0.00'; ws.getCell(tr,13).numFmt='$#,##0.00'; ws.getCell(tr,13).font={bold:true,color:{argb:"FF087A3E"}};
  borderRange(ws,16,tr,1,13);

  [10,28,30,16,14,14,13,13,12,14,15,16,17].forEach((w,i)=>ws.getColumn(i+1).width=w);
...
function addPLUSheet(wb,p,period){
  ...
  const info=[
    ...
    ["Redemption Amount",+p.rebate_amount],["Program Period",`${excelDate(p.start_date)} - ${excelDate(p.end_date)}`]
  ];
  ...
  const hdr=["Date / Period","Product Code (PLU)","Qty Sold","Rebate Qualified Qty","Item Price (Each)","Sales Price (Total)","Notes"];
  const hrow=ws.getRow(16);hdr.forEach((h,i)=>hrow.getCell(i+1).value=h);styleHeaderRow(hrow);
  const sr=17; [period.name,p.plu,+p.qty_sold,+p.rebate_qualified||0,+p.item_price,c.sales,p.notes||""].forEach((v,i)=>ws.getCell(sr,i+1).value=v);
  ws.getCell(sr,5).numFmt='$#,##0.00';ws.getCell(sr,6).numFmt='$#,##0.00';borderRange(ws,16,17,1,7);

  ws.mergeCells("A19:C19");ws.getCell("A19").value="SALES & REDEMPTION SUMMARY";ws.getCell("A19").font={bold:true,color:{argb:"FFFFFFFF"}};ws.getCell("A19").fill={type:"pattern",pattern:"solid",fgColor:{argb:"FF0F6D73"}};
  const sum=[["Total Qty Sold",+p.qty_sold],["Rebate Qualified Qty",+p.rebate_qualified||0],["Units Required per Redemption",+p.units_required],["Qualifying Redemption Earned",c.qual],["Redemption Amount per Qualifying Purchase",+p.rebate_amount],["TOTAL REDEMPTION DUE",c.due]];
  sum.forEach((x,i)=>{let r=20+i;ws.mergeCells(r,1,r,2);ws.getCell(r,1).value=x[0];ws.getCell(r,3).value=x[1];if(i>=4)ws.getCell(r,3).numFmt='$#,##0.00';if(i===5){for(let col=1;col<=3;col++){ws.getCell(r,col).fill={type:"pattern",pattern:"solid",fgColor:{argb:"FF0B7A46"}};ws.getCell(r,col).font={bold:true,color:{argb:"FFFFFFFF"},size:col===3?15:11};}}});
  borderRange(ws,19,25,1,3);

  [20,28,14,18,18,20,32].forEach((w,i)=>ws.getColumn(i+1).width=w);
